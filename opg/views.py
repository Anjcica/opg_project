        ...
        if user_form.is_valid() and opg_form.is_valid():
            ...
            for name in Opg.objects.all().values_list("name"):
                ...
                if opg.name.upper() == name[0].upper():
                    messages.error(request, 'Opg with this Name already exists.')
...
def add_product(request):
    ...
    ProductFormSet = inlineformset_factory(Opg, Product, fields=('name', 'category'), extra=7, can_delete=False)
